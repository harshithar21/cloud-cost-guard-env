"""
inference.py — CloudCostGuardEnv baseline inference script
Required by hackathon. Uses OpenAI client. Reads credentials from env vars.
Emits [START], [STEP], [END] logs exactly as specified by the validator.
"""
import asyncio
import json
import os
import sys
import time
from typing import List

import httpx
from openai import OpenAI

# ── Config ────────────────────────────────────────────────────────────────────
API_BASE_URL = os.getenv("API_BASE_URL", "https://router.huggingface.co/v1")
API_KEY      = os.getenv("HF_TOKEN", "")
MODEL_NAME   = os.getenv("MODEL_NAME", "meta-llama/Llama-3.2-3B-Instruct")
TEMPERATURE  = 0.3
MAX_TOKENS   = 512

ENV_BASE_URL = os.getenv("ENV_BASE_URL", "http://localhost:8000")

TASKS        = ["task_easy", "task_medium", "task_hard"]
MAX_STEPS    = {"task_easy": 15, "task_medium": 20, "task_hard": 25}
MAX_TOTAL_REWARD_PER_TASK = 3.0
SUCCESS_THRESHOLD = 0.5

# ── Logging helpers — EXACT format the validator parses ───────────────────────
def log_start(task: str, env: str, model: str):
    print(f"[START] task={task} env={env} model={model}", flush=True)

def log_step(step: int, action: dict, reward: float, done: bool, error=None):
    action_str = json.dumps(action)
    error_str  = str(error) if error else "null"
    print(f"[STEP] step={step} action={action_str} reward={reward:.4f} done={done} error={error_str}", flush=True)

def log_end(success: bool, steps: int, score: float, rewards: List[float]):
    rewards_str = json.dumps(rewards)
    print(f"[END] success={success} steps={steps} score={score:.4f} rewards={rewards_str}", flush=True)


# ── Wait for server to be ready ───────────────────────────────────────────────
async def wait_for_server(base_url: str, timeout: int = 60) -> bool:
    """Poll /health or / until server responds. Returns True if ready."""
    deadline = time.time() + timeout
    async with httpx.AsyncClient(timeout=5.0) as client:
        while time.time() < deadline:
            try:
                resp = await client.get(f"{base_url}/health")
                if resp.status_code < 500:
                    return True
            except Exception:
                pass
            try:
                resp = await client.get(f"{base_url}/")
                if resp.status_code < 500:
                    return True
            except Exception:
                pass
            await asyncio.sleep(2)
    return False


# ── LLM call ──────────────────────────────────────────────────────────────────
def get_action(client: OpenAI, obs: dict, step: int, task_id: str) -> dict:
    """Ask the LLM to choose an action given the current observation."""
    pods_summary = "\n".join([
        f"  - {p['name']}: cpu_req={p['cpu_requested']} cpu_used={p['cpu_used']} "
        f"mem_req={p['mem_requested_mb']}MB mem_used={p['mem_used_mb']}MB "
        f"replicas={p['replicas']} spot={p['on_spot']} cost=${p['cost_per_hour']}/hr"
        for p in obs.get("pods", [])
    ])

    prompt = f"""You are a Kubernetes FinOps agent. Your goal is to reduce cloud costs while maintaining SLAs.

Current cluster state (step {step}):
- Total cost: ${obs.get('total_cost_per_hour', 0)}/hr (baseline: ${obs.get('baseline_cost_per_hour', 0)}/hr)
- Cost saved so far: {obs.get('cost_saved_percent', 0)}%
- SLA violations: {obs.get('sla_violations', 0)}
- Response time: {obs.get('avg_response_time_ms', 0)}ms (SLA limit: {obs.get('sla_threshold_ms', 500)}ms)
- Budget remaining: ${obs.get('budget_remaining', 0):.2f} of ${obs.get('budget_limit', 0):.2f}
- Last message: {obs.get('message', '')}

Pods:
{pods_summary}

Available actions:
- set_request_cpu: target=<pod_name>, value=<cpu_cores>
- set_request_memory: target=<pod_name>, value=<mb>
- set_replicas: target=<pod_name>, value=<count>
- migrate_to_spot: target=<pod_name>, value=0
- migrate_to_ondemand: target=<pod_name>, value=0
- noop: target=none, value=0

Respond ONLY with a JSON object:
{{"action_type": "...", "target": "...", "value": <number>, "reasoning": "..."}}

Pick the action that saves the most cost without violating SLAs."""

    try:
        resp = client.chat.completions.create(
            model=MODEL_NAME,
            messages=[{"role": "user", "content": prompt}],
            temperature=TEMPERATURE,
            max_tokens=MAX_TOKENS,
        )
        text = (resp.choices[0].message.content or "").strip()
        # Extract JSON from response
        if "```" in text:
            text = text.split("```")[1].replace("json", "").strip()
        action = json.loads(text)
        if "action_type" not in action:
            raise ValueError("Missing action_type")
        return action
    except Exception as e:
        print(f"[DEBUG] LLM call failed: {e}", flush=True)
        # Fallback: find most wasteful pod and right-size it
        pods = obs.get("pods", [])
        if pods:
            worst = max(pods, key=lambda p: p.get("cpu_waste_ratio", 0))
            return {
                "action_type": "set_request_cpu",
                "target": worst["name"],
                "value": round(worst.get("cpu_used", 0.5) * 1.2, 2),
                "reasoning": "fallback: right-size most wasteful pod"
            }
        return {"action_type": "noop", "target": "none", "value": 0, "reasoning": "fallback noop"}


# ── Safe HTTP helpers ─────────────────────────────────────────────────────────
async def safe_reset(client: httpx.AsyncClient, base_url: str, task_id: str,
                     retries: int = 5, delay: float = 3.0) -> dict:
    """POST /reset with retry. Never raises — returns empty obs on total failure."""
    last_err = None
    for attempt in range(retries):
        try:
            resp = await client.post(f"{base_url}/reset", json={"task_id": task_id})
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
            last_err = e
            print(f"[DEBUG] reset attempt {attempt+1}/{retries} failed: {e}", flush=True)
            if attempt < retries - 1:
                await asyncio.sleep(delay)
    # Total failure — return a safe default so the script doesn't crash
    print(f"[DEBUG] All reset attempts failed. Returning empty state.", flush=True)
    return {"observation": {}, "done": False, "reward": 0.0}


async def safe_step(client: httpx.AsyncClient, base_url: str, action: dict,
                    retries: int = 3, delay: float = 2.0) -> dict:
    """POST /step with retry. Never raises — returns 0 reward on total failure."""
    for attempt in range(retries):
        try:
            resp = await client.post(f"{base_url}/step", json={"action": action})
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
            print(f"[DEBUG] step attempt {attempt+1}/{retries} failed: {e}", flush=True)
            if attempt < retries - 1:
                await asyncio.sleep(delay)
    return {"observation": {}, "done": True, "reward": 0.0}


# ── Run one task episode ──────────────────────────────────────────────────────
async def run_task(task_id: str, client: OpenAI, base_url: str) -> float:
    """Run one full episode for a task. Returns normalized score 0.0–1.0."""
    max_steps = MAX_STEPS[task_id]
    rewards: List[float] = []
    steps_taken = 0
    score = 0.0
    success = False

    log_start(task=task_id, env="cloud_cost_guard_env", model=MODEL_NAME)

    try:
        async with httpx.AsyncClient(timeout=30.0) as http_client:
            result = await safe_reset(http_client, base_url, task_id)
            obs = result.get("observation", {})

            for step in range(1, max_steps + 1):
                if obs.get("done", False):
                    break

                action = get_action(client, obs, step, task_id)

                result = await safe_step(http_client, base_url, action)
                obs    = result.get("observation", {})
                reward = float(result.get("reward", 0.0))
                done   = result.get("done", False)

                rewards.append(reward)
                steps_taken = step

                log_step(step=step, action=action, reward=reward, done=done, error=None)

                if done:
                    break

        total_reward = sum(rewards)
        score   = min(1.0, max(0.0, total_reward / MAX_TOTAL_REWARD_PER_TASK))
        success = score >= SUCCESS_THRESHOLD

    except Exception as e:
        # Catch-all: log and treat as zero score — script must NOT crash
        print(f"[DEBUG] run_task({task_id}) unexpected error: {e}", flush=True)
        score   = 0.0
        success = False

    log_end(success=success, steps=steps_taken, score=score, rewards=rewards)
    return score


# ── Main ──────────────────────────────────────────────────────────────────────
async def main():
    base_url = ENV_BASE_URL
    print(f"[DEBUG] ENV_BASE_URL={base_url}", flush=True)

    # Wait for the environment server to be ready before running tasks
    print("[DEBUG] Waiting for environment server...", flush=True)
    ready = await wait_for_server(base_url, timeout=90)
    if not ready:
        print(f"[DEBUG] Server not ready at {base_url} after 90s — attempting anyway.", flush=True)

    llm_client = OpenAI(base_url=API_BASE_URL, api_key=API_KEY or "dummy")
    all_scores = {}

    for task_id in TASKS:
        print(f"\n{'='*50}", flush=True)
        print(f"Running task: {task_id}", flush=True)
        score = await run_task(task_id, llm_client, base_url)
        all_scores[task_id] = score
        print(f"Task {task_id} score: {score:.4f}", flush=True)

    print(f"\n{'='*50}", flush=True)
    print("BASELINE SCORES:", flush=True)
    for task, s in all_scores.items():
        print(f"  {task}: {s:.4f}", flush=True)
    avg = sum(all_scores.values()) / len(all_scores)
    print(f"  AVERAGE: {avg:.4f}", flush=True)


if __name__ == "__main__":
    asyncio.run(main())